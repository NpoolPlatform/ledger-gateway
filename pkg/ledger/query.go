...
func (h *queryHandler) prepareAppLedgers(ctx context.Context) error {
	...
	for _, ledgers := range h.ledgers {
		for coinTypeID, _ := range ledgers {
...
func (h *queryHandler) prepareUserLedgers(ctx context.Context) error {
	...
	coinTypeIDs := []string{}
	for coinTypeID, _ := range h.appCoins {
		coinTypeIDs = append(coinTypeIDs, coinTypeID)
	}
	// Get coin ledgers
...
func (h *queryHandler) formalize(ledger *ledgermwpb.Ledger, coin *coinmwpb.Coin, user *usermwpb.User) {
...
func (h *queryHandler) formalizeAppLegers() {
...
func (h *Handler) GetLedgers(ctx context.Context) ([]*npool.Ledger, uint32, error) {
	...
	if h.UserID == nil {
		if err := h.prepareAppLedgers(ctx); err != nil {
			return nil, 0, err
		}
	} else {
		if err := h.prepareUserLedgers(ctx); err != nil {
			return nil, 0, err
		}
	}
